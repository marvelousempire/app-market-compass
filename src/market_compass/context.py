...
RISK = {"fed", "rate", "inflation", "shutdown", "war", "attack", "earnings", "lawsuit", "regulation", "hack", "unlock", "election"}
NARRATIVES = {
...
def foundation_layer(x: pd.DataFrame, quote: dict) -> LayerResult:
    ...
    return LayerResult(key="foundation", label="Asset Reality & Quality", state=state, score=float(np.clip(score, -1, 1)), confidence=.72 if len(missing) < 2 else .45, evidence=ev, counter_evidence=ce, metrics={"history_days": history_days, "median_dollar_volume_30d": dollar_vol, "market_cap": market_cap, "quote_type": quote.get("quoteType")}, missing=missing)


def news_layer(x: pd.DataFrame, news: list[dict], symbol: str) -> LayerResult:
    if not news:
        return LayerResult(key="news", label="Human Factor & News", state="no current news data", score=0, confidence=.15, missing=["headlines"])
    now = datetime.now(timezone.utc)
    scored, risk_hits = [], []
    for n in news[:20]:
        s = _sentiment(n.get("title", ""))
        try:
    ...
    top_pos = sorted(scored, reverse=True, key=lambda z: z[0])[:2]
    top_neg = sorted(scored, key=lambda z: z[0])[:2]
    return LayerResult(
        key="news", label="Human Factor & News", state="positive" if score > .1 else "negative" if score < -.1 else "mixed or quiet",
        score=score, confidence=min(.8, .35 + len(news) / 40),
        ...
        metrics={"headline_count": len(news), "sentiment": net, "latest_daily_return": daily, "market_reaction_confirms": bool(confirms), "event_risk_headlines": risk_hits[:5]},
    )
...
def historical_layer(x: pd.DataFrame, horizon: int = 20) -> LayerResult:
    ...
    _dist, idx = nn.kneighbors(scaler.transform(f.iloc[[-1]]))
    ...
    return LayerResult(
        ...
        score=score, confidence=min(.82, .35 + k / 20),
        ...
        counter_evidence=[Evidence(text=f"Counterexample {dt.date()}: {val:+.1%}.", direction=-1 if score >= 0 else 1, strength=.55) for dt, val in counter.sort_values().head(3).items()],
        metrics={"analog_count": k, "mean_forward_return": mean_ret, "median_forward_return": float(outcomes.median()), "positive_rate": float((outcomes > 0).mean()), "dates": [d.isoformat() for d in analog_dates]},
    )
...
def forecast(x: pd.DataFrame, horizon: int = 20) -> dict:
    ...
    beats = mae < base
    confidence = float(np.clip(.3 + .5 * max(0, 1 - mae / base), .2, .8)) if base else .2
    return {"state": "usable" if beats else "baseline_not_beaten", "expected_return": expected if beats else None, "raw_model_return": expected, "cv_mae": mae, "baseline_mae": base, "beats_baseline": beats, "confidence": confidence}


def relationship_and_narrative(news: list[dict], symbol: str) -> tuple[LayerResult, LayerResult, dict]:
    g = nx.Graph()
    g.add_node(symbol, type="asset")
    ...
    for a, b, d in g.edges(data=True):
        ...
        edges.append({"from": a, "to": b, "type": d.get("type"), "inferred": True, "mentions": d.get("mentions", 1), "sentiment": avg, "source": d.get("source")})
    paths = []
    ...
    rel = LayerResult(
        key="relationships", label="Relationship Intelligence", state="inferred evidence graph",
        score=float(np.clip(rel_net * .4, -1, 1)), confidence=.48 if edges else .15,
        evidence=[Evidence(text=f"{e} is connected through current related news.", direction=1, strength=.35, source="inferred co-mention") for e, vals in list(entity_scores.items())[:4] if np.mean(vals) >= 0],
        counter_evidence=[Evidence(text=f"{e} is connected through negatively toned related news.", direction=-1, strength=.35, source="inferred co-mention") for e, vals in list(entity_scores.items())[:4] if np.mean(vals) < 0],
        metrics={"edge_count": len(edges), "path_count": len(paths)},
    ...
    nar = LayerResult(key="narrative", label="Market Narrative", state=f"{dominant}: {stage}", score=float(np.clip(avg * min(1, n / 4), -1, 1)), confidence=min(.75, .25 + n * .1), evidence=[Evidence(text=f"Dominant story: {dominant} with {n} related headlines.", direction=1 if avg >= 0 else -1, strength=min(.8, .3 + n * .08))] if n else [], counter_evidence=[Evidence(text="A crowded story can reverse even when the headlines still look good.", direction=-1, strength=.35)] if n >= 5 and avg > 0 else [], metrics={"counts": counts, "dominant": dominant, "stage": stage, "sentiment": avg})
    board = {"nodes": [{"id": node, **attrs} for node, attrs in g.nodes(data=True)], "edges": edges, "paths": paths[:30], "warning": "News co-mentions are inferred relationships, not proof of causation."}
