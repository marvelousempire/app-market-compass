...
def test_v03_workbench_surface_is_served():
    ...
    for text in ["TRADING WORKBENCH", "RSI 14", "MACD", "Evidence Contributions", "Catalyst Timeline", "Evidence Board"]:
        assert text in page.text
    assert "lightweight-charts" in page.text

    script = client.get("/static/app.js")
    assert script.status_code == 200
    for function in ["renderCharts", "renderContributions", "renderCatalysts", "saveSnapshot"]:
